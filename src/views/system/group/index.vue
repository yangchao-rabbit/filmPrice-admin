...
import { type Group, GroupDeleteAPI, GroupListAPI } from '@/api/system.ts'
import { ElMessage, ElMessageBox } from 'element-plus'
import { DateFormat } from '@/utils/format.ts'
import { DiaType, type IDia } from '@/types/diaType.ts'
import DialogModel from '@/views/system/group/DialogModel.vue'
...
const Index = reactive({
    ...
    columns: [
        ...
        { label: '创建时间', default: false, formatter: (row: any) => DateFormat(row.created_at) },
        { label: '更新时间', default: false, formatter: (row: any) => DateFormat(row.updated_at) },
        { label: '描述', prop: 'desc' },
    ],
    data: [] as Group[],
    loading: false,
    ...
    onDelete: (row: any) => {
        ElMessageBox.confirm(`是否删除 [${row.name}] 组?`, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning',
        })
            .then(async () => {
                const resp = await GroupDeleteAPI(row.id)
                if (resp.code === 0) {
