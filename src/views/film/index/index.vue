...
import { type Film, FilmDeleteAPI, FilmListAPI } from '@/api/film.ts'
...
const Index = reactive({
    ...
    columns: [
        ...
        { label: '别名', prop: 'alias' },
        { label: '品牌', prop: 'brand' },
        { label: 'ISO', prop: 'iso', sortable: true },
        { label: '类型', prop: 'type' },
        { label: '格式', prop: 'format', sortable: true },
        { label: '图片', prop: 'image', default: false },
    ...
    onDelete: (row: any) => {
        ElMessageBox.confirm(`是否删除 [${row.name}] 胶片?`, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning',
        })
            .then(async () => {
                const resp = await FilmDeleteAPI(row.id)
                if (resp.code === 0) {
